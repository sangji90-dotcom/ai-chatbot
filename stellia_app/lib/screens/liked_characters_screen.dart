...
class LikedCharactersScreen extends StatefulWidget {
  const LikedCharactersScreen({super.key});

  @override
  State<LikedCharactersScreen> createState() => _LikedCharactersScreenState();
}

class _LikedCharactersScreenState extends State<LikedCharactersScreen> {
  ...
  Future<void> _load() async {
    try {
      final data = await ApiService.getLikedCharacters();
      setState(() => _characters = data.map((e) => Character.fromJson(e)).toList());
    } catch (e) {
      debugPrint('좋아요 캐릭터 로드 실패: $e');
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF0A0A0F),
        elevation: 0,
        title: const Text('좋아요한 캐릭터', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
      ...
      body: _loading
          ...
          : _characters.isEmpty
              ? const Center(child: Text('좋아요한 캐릭터가 없어요.', style: TextStyle(color: Color(0xFF6B7280))))
              : RefreshIndicator(
